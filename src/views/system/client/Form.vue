...
import { RegisteredClient, AuthenticationMethodEnum } from "@/entity/system/client";
...
watch(authorizationGrantTypes, () => {
    formData.value.authorizationGrantTypes = authorizationGrantTypes.value.join(",");
})
...
function handleComfirm() {
    form.value.validate((valid: boolean) => {
        if (valid) {
            ...
            saveOrUpdate(formData.value, props.isEdit).then(() => {
...
onMounted(() => {
    if (props.isEdit) {
        ...
        if (formData.value.authorizationGrantTypes) {
            authorizationGrantTypes.value = formData.value.authorizationGrantTypes.split(',') as string[]
        }
    }
});
</script>
...
<template>
    <el-dialog v-model="dialogVisible" draggable :title="title" :before-close="closeDialog" width="800px">
        <el-divider content-position="left">
            <span style="color: red;">提示：如果有多个 SCOPE 或 重定向 URI，请使用逗号分隔；新建客户端默认密码为 123456；</span>
        </el-divider>
            ...
            <el-row :gutter="20">
                <el-col :span="12">
                    <el-form-item label="认证方式" prop="clientAuthenticationMethods">
                        <el-select v-model="formData.clientAuthenticationMethods" placeholder="请选择"
                            style="width: 450px">
                            <el-option v-for="item in AuthenticationMethodEnum" :key="item" :label="item"
                                :value="item" />
                        </el-select>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="授权类型" prop="authorizationGrantTypes">
                        <el-select v-model="authorizationGrantTypes" multiple collapse-tags collapse-tags-tooltip
                            placeholder="请选择" style="width: 450px">
                            <el-option v-for="item in AuthorizationGrantTypeOptions" :key="item.value"
            ...
            <el-row :gutter="20">
                <el-col :span="12">
                    <el-form-item label="SCOPE" prop="scopes">
                        <el-input type="textarea" v-model="formData.scopes" autocomplete="off" />
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="重定向URI" prop="redirectUris">
                        <el-input type="textarea" v-model="formData.redirectUris" autocomplete="off" />
                    </el-form-item>
                </el-col>
            </el-row>
            <el-row :gutter="20">
                ...
                <el-col :span="12">
                    <el-form-item label="访问令牌格式" prop="accessTokenFormat">
                        <el-select v-model="formData.accessTokenFormat" placeholder="请选择" style="width: 450px">
            ...
            <el-row :gutter="20">
                <el-col :span="12">
                    <el-form-item label="访问令牌有效期" prop="accessTokenTimeToLive">
                        <el-input-number v-model="formData.accessTokenTimeToLive" :min="1" controls-position="right" />
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="刷新令牌有效期" prop="refreshTokenTimeToLive">
                        <el-input-number v-model="formData.refreshTokenTimeToLive" :min="1" controls-position="right" />
                    </el-form-item>
                </el-col>
            </el-row>
        </el-form>
        <template #footer>
            <span class="dialog-footer">
                ...
                <el-button type="primary" @click="handleComfirm" :loading="confirmLoading">确认</el-button>
            </span>
        </template>
    </el-dialog>
</template>
