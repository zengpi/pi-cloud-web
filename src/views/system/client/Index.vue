...
import { type RegisteredClient, AuthorizationGrantTypeEnum, FormDialog } from '@/entity/system/client'

import { getClients, delClient, saveOrUpdate } from "@/api/system/client"
...
const query = ref<BaseQueryParam>(new BaseQueryParam())
...
const resetPassFormData = ref<{ id?: number, clientSecret?: string, clientSecretConfirm?: string }>({})

const validConfirmPass = (rule: any, value: any, callback: any) => {
    if (resetPassFormData.value.clientSecret !== resetPassFormData.value.clientSecretConfirm) {
...
function handleDel(index: number, row: RegisteredClient) {
    ElMessageBox.confirm("将永久删除选中的记录，是否继续？", "警告", {
        ...
        delClient(row.id.toString()).then(() => {
            ElMessage({
                type: "success",
                message: "删除成功"
            })
...
function handleDelBatch() {
    ...
    ElMessageBox.confirm("将永久删除选中的记录，是否继续？", "警告", {
        ...
        delBtnLoading.value = true;
        const ids = rows.map((e: RegisteredClient) => e.id);
        delClient(ids.join(",")).then(() => {
            ElMessage({
                type: "success",
                message: "删除成功"
            });
...
function handleAuthorizationGrantTypes(authorizationGrantTypes: string) {
    ...
    authorizationGrantTypeArr.forEach((item) => {
        if (rst) rst += " | ";
        rst += AuthorizationGrantTypeEnum[item as keyof typeof AuthorizationGrantTypeEnum]
    })
    return rst;
}
...
function handleResetPassComfirm() {
    form.value.validate((valid: boolean) => {
        if (valid) {
            ...
            saveOrUpdate(resetPassFormData.value, true).then(() => {
...
<template>
    <div class="app-container">
        <div class="head-container">
            <div class="query">
                <template v-if="showSearch">
                    <el-input v-model="query.keyWord" clearable placeholder="客户端ID" class="query-item"
            ...
            <div class="tools">
                <div class="tools-left">
                    ...
                    <el-button v-has-authority="['sys_user_delete']" type="danger" :icon="Delete"
                        :loading="delBtnLoading" class="tool-item" @click="handleDelBatch">
                        删除
                    </el-button>
                </div>
        ...
        <el-table ref="table" :data="tableData" v-loading="loading" stripe border>
            ...
            <el-table-column prop="clientName" label="名称" width="110" show-overflow-tooltip />
            <el-table-column prop="clientAuthenticationMethods" label="认证方式" width="150" show-overflow-tooltip />
            <el-table-column prop="authorizationGrantTypes" label="授权类型" min-width="220" show-overflow-tooltip
                #default="{ row }">
                <span>{{ handleAuthorizationGrantTypes(row.authorizationGrantTypes) }}</span>
            </el-table-column>
            <el-table-column prop="redirectUris" label="重定向URI" width="200" show-overflow-tooltip />
            <el-table-column prop="scopes" label="作用域" width="90" show-overflow-tooltip />
            <el-table-column prop="requireAuthorizationConsent" label="授权许可" width="90" #default="{ row }">
                <span v-if="row.requireAuthorizationConsent === 1">是</span>
                <span v-else>否</span>
            </el-table-column>
            <el-table-column prop="accessTokenFormat" label="访问令牌格式" width="110" #default="{ row }">
                <span v-if="row.accessTokenFormat === 'self-contained'">JWT</span>
                <span v-else-if="row.accessTokenFormat === 'reference'">Opaque</span>
            </el-table-column>
            <el-table-column prop="accessTokenTimeToLive" label="访问令牌有效期(s)" width="150" />
            <el-table-column prop="refreshTokenTimeToLive" label="刷新令牌有效期(s)" width="150" />
            <el-table-column fixed="right" label="操作" width="220px" align="center">
                <template #default="scope">
                    <el-tooltip content="重置密码" placement="top">
                        <el-button type="warning" :icon="Lock" @click="handleResetPass(scope.row.id)" />
                    </el-tooltip>
        ...
        <el-dialog v-if="resetPassDialogVisible" v-model="resetPassDialogVisible" draggable title="重置密码"
            :before-close="handleCloseResetPassDialog" width="400px">
            <el-form ref="form" :model="resetPassFormData" :rules="resetPassFormRules" label-width="90px">
                <el-form-item label="密码" prop="clientSecret">
                    <el-input v-model="resetPassFormData.clientSecret" type="password" placeholder="请输入密码"
                        show-password />
                </el-form-item>
                <el-form-item label="确认密码" prop="clientSecretConfirm">
                    <el-input v-model="resetPassFormData.clientSecretConfirm" type="password" placeholder="请在次输入密码"
                        show-password />
                </el-form-item>
            </el-form>
            <template #footer>
                <span class="dialog-footer">
                    ...
                    <el-button type="primary" @click="handleResetPassComfirm" :loading="resetPassConfirmLoading">确认
                    </el-button>
                </span>
            </template>
        </el-dialog>
